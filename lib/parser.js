...
Parser.command = function(messageEmitter, data, gameMaster) {
	...
	var attack = function(messageEmitter, data, gameMaster) {
		...
		targetName = helper.stringMatch(targetName, targArray);
		
		// targArray.forEach(function (potentialTarget) {
		// 	if (helper.stringMatch(targetName, potentialTarget)) 
		// 	{
		...
		helper.roomTalk(room.id, gameMaster, outcome);
	}
	...
	if (/^'.*/.test(data)) {
		...
		helper.roomTalk(messageEmitter.room, gameMaster, message);
