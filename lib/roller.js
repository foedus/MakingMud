...
roller.pickStats = function (messageEmitter, socket, newUser, rollerState) {
	...
	var strength = 50 + Math.ceil(Math.random() * 20),
		constitution = 80 + Math.ceil(Math.random() * 20),
		agility = 70 + Math.ceil(Math.random() * 20),
		aptitude = 70 + Math.ceil(Math.random() * 20),
		focus = 50 + Math.ceil(Math.random() * 20),
		logic = 80 + Math.ceil(Math.random() * 20),
		creativity = 30 + Math.ceil(Math.random() * 70),
	...
	if (rollerState === "pickStats") {
		messageEmitter.once('command', function(data) {
			if (data.data === "y") {
				var message = {
					"type": "roller",
					"content": "Stats confirmed."
				};
				// Should probably show player the final character sheet, ask them to confirm or start over.
