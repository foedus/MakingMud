	...
	if (rollerState === "pickStats") {
		messageEmitter.once('command', function(data) {
			if (data.data === "y") {
				var message = {
					"type": "roller",
					"content": "Stats confirmed. Assigning stats randomly."
				};
				// Should probably show player the final character sheet, ask them to confirm or start over.
				// for now it's just random.
				
				// assign user stats
				newUser.stats.strength = roll1;
				newUser.stats.constitution = roll2;
				newUser.stats.agility = roll3;
				newUser.stats.aptitude = roll4;
				newUser.stats.focus = roll5;
				newUser.stats.logic = roll6;
				newUser.stats.creativity = roll7;
				messageEmitter.emit('OUT', message);
